function UITestQuestion(currentQuestion) {
  ...
  // Перебираем варианты ответов текущего вопроса
  const optionsHTML = currentQuestion.options.map((option, index) => {
    ...
    return `
      <div class="mb-2">
        ...
        <input type="checkbox" class="btn-check" id="${optionId}" value="${option}" autocomplete="off">
    ...
    `;
          ...
          //  <span class="badge rounded-pill bg-light text-light border ms-2">${index + 1}</span>
  return `
    <div class="card bg-dark t shadow-sm border-0 my-3 test-question-container w-100">
      <div class="card-body py-4 px-0">
        ...
        <div class="d-flex justify-content-end mb-4" id="confirmBlock">
          <button class="btn btn-primary px-4 fw-semibold" id="btnConfirmAnswer">
            Подтвердить выбор
          </button>
        </div>
 
        <!-- Подсказка с правильным ответом (скрыта по умолчанию) -->
        <div class="alert alert-info border-0 bg-info bg-opacity-10 mb-4 d-none" id="correctAnswerBlock">
        ...
        <div class="d-flex justify-content-end d-none" id="nextQuestionBlock">
          <button class="btn btn-primary px-4 py-2 d-inline-flex align-items-center fw-semibold" id="btnNextQuestion">
            <span>Дальше</span>
